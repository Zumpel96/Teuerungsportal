...
{
    private const string LidlStoreId = "bdf1f1ad-84d5-4cfa-a313-c18e8899cb2f";
    ...
    public LidlDataExtractor(IAsyncCollector<Product> dbProducts, IAsyncCollector<Price> dbPrices)
    {
    ...
    public async Task Run()
    {
        ...
        // Iterate over Data
        foreach (var hit in responseData)
        {
            var data = hit;
            ...
            var parseSuccess = double.TryParse(data["price"]["price"].ToString(), out double newPriceValue);
            if (!parseSuccess)
            {
                continue;
            }
    ...
    private async Task<Guid?> GetProduct(string articleNumber)
    {
        ...
        existsCommand.Parameters.AddWithValue("@storeId", LidlStoreId);
