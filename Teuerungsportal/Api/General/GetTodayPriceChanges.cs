...
public static class GetTodayPriceChanges
{
    [FunctionName("GetTodayPriceChanges")]
    public static IActionResult Run(
        [HttpTrigger(AuthorizationLevel.Anonymous, "get", Route = "prices/today")] HttpRequest req,
        [Sql(
                commandText: @"
                             WITH [previous_prices] AS (
                                ...
                                FROM 
                                  [previous_prices] 
                                WHERE [timestamp] >= DATEADD(day,-1,GETDATE()) AND [previousValue] IS NOT NULL
                                ORDER BY 
                                  [timestamp] DESC;",
