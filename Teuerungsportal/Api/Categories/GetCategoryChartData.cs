...
public static class GetCategoryChartData
{
    [FunctionName("GetCategoryChartData")]
    public static IActionResult Run(
        [HttpTrigger(AuthorizationLevel.Anonymous, "get", Route = "categories/{categoryId}/chart")] HttpRequest req,
        [Sql(
                commandText: @"
                                ...
                                [grouped_prices] AS
                                  (SELECT [pr].[storeId],
                                     ...
                                     (SELECT CASE
                                                 ...
                                                 ELSE 1
                                             END AS [count]
                                      FROM [dbo].[GetCategoryProducts](@categoryId) [pi]
                                      WHERE [pi].[storeId] = [pr].[storeId]) AS [inflationValue]
                                   FROM [previous_prices] [p]
                                   JOIN [dbo].[GetCategoryProducts](@categoryId) [pr] ON [p].[productId] = [pr].[id]
