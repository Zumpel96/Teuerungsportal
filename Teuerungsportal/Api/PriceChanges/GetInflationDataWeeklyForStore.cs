...
public static class GetInflationDataWeeklyForStore
{
    [FunctionName("GetInflationDataWeeklyForStoreV2")]
    public static IActionResult Run(
        [HttpTrigger(AuthorizationLevel.Anonymous, "get", Route = "v2/prices/inflation/store/{storeId}/week")] HttpRequest req,
        [Sql(
                commandText: @"
                        ...
                        [grouped_prices] AS (
                          ...
                          WHERE 
                            [timestamp] >= CAST(GETDATE() - 6 AS DATE) 
                            AND [s].[hidden] = 0 
    ...
    {
      var countList = inflationObjects.ToList();
