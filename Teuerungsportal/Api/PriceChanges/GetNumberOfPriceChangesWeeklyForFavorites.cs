...
public static class GetNumberOfPriceChangesWeeklyForFavorites
{
                                ...
                                (
                                  ...
                                  WHERE 
                                    [previousValue] IS NOT NULL
									                  AND [timestamp] >= CAST(GETDATE() - 6 AS DATE)
                                ) AS sub 
                              WHERE 
                                [sub].[storeId] = [s].[id]
                            ) AS [Count], 
    ...
    [FunctionName("GetNumberOfPriceChangesWeeklyForFavoritesV2")]
