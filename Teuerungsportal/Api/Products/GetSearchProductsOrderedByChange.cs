...
public static class GetSearchProductsOrderedByChange
{
    [FunctionName("GetSearchProductsOrderedByChangeV2")]
    public static IActionResult Run(
        [HttpTrigger(AuthorizationLevel.Anonymous, "get", Route = "v2/products/search/{searchString}/{page}/change/{storeNames}")] HttpRequest req,
        [Sql(
                commandText: @"
                          WITH [previous_prices] AS (
                            ...
                            ORDER BY
                              (CASE WHEN [previousValue] IS NULL THEN 0 ELSE [currentValue] - [previousValue] END)
